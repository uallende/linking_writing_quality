...
def process_action_time_activity(logs):
    def create_action_time_activity_features(logs):
        df_logs = logs.copy()
    ...
    action_time_activity = create_action_time_activity_features(logs)
    return action_time_activity

def process_feats_action_time_gap(logs):
    def calc_action_time_gap(logs):
        action_time_gap_df = logs.copy()
    ...
    action_time_gap_feats = calc_action_time_gap(logs).pivot(index='id', columns='level_1', values='action_time_gap').reset_index()
    return action_time_gap_feats

def process_feats_time_gap_activity(logs):
    def calc_time_gap_activity(comb_df):
        ...
        action_time_gap_stats = grouped.apply(lambda x: diverse_stats(x, 'atg')).reset_index()
    ...
    feats = calc_time_gap_activity(logs)

    new_columns = [col[1] if col[0] == 'id' else '_'.join(col) for col in feats.columns]
    new_columns[0] = 'id'
    feats.columns = new_columns
    return feats
...
def process_adjusted_eff_time(logs):
    def create_aggregations(df):
        # Your existing aggregation logic
        agg_df = df.groupby("id")[['eff_time', 'eff_time_gap']].agg(
            ['mean', 'std', 'max', 'last', 'first', 'sem', 'median', 'sum'])
        agg_df.columns = ['_'.join(x) for x in agg_df.columns]
        agg_df = agg_df.add_prefix("tmp_")
        agg_df.reset_index(inplace=True)
        return agg_df
    
    def create_adjusted_eff_time_df(logs):
        eff_time_df = logs[['id', 'down_time', 'up_time', 'action_time', 'event_id']].sort_values(['id', 'event_id']).copy()
        ...
        eff_time_df = eff_time_df.fillna(0)
        return eff_time_df

    efficient_df = create_adjusted_eff_time_df(logs)
    feats = create_aggregations(efficient_df)
    return feats

def process_re_cut_essays(logs):
    def filter_and_transform_re_cut(logs_df):
        re_cut_df = logs_df[logs_df['activity'] == 'Remove/Cut'].copy()
        re_cut_df['activity'] = 'Input'
        return re_cut_df
    
    re_cut = filter_and_transform_re_cut(logs)
    re_cut_df = getEssays(re_cut)
    cut_words = create_word_length_features(re_cut_df, 'essay', 'id', 're_cut')
    return cut_words
...
def wpm_feats(logs):
    ...
    return grouped_logs.reset_index()